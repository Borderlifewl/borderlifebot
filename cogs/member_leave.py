...
class MemberLeave(commands.Cog):
    ...
    async def on_member_remove(self, member):
        ...
        if log_channel:
            now_paris = datetime.now(pytz.utc).astimezone(pytz.timezone('Europe/Paris'))
            embed = discord.Embed(
            ...
            embed.add_field(name="Date de départ du joueur", value=now_paris.strftime('%d/%m/%Y à %H:%M:%S'), inline=False)
